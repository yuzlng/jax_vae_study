...
class VAE(nn.Module):
    ...
    def __call__(self, x):
        # CODE HERE
        ## 1. amortized_inference 
        mean_z, logvar_z, z_shape = self.encoder(x)       

        ## 2. pathwise_estimator (or reparametrization trick)
        std = jnp.exp(0.5 * logvar_z)
        epsilon = random.normal(self.rng_generator(), mean_z.shape)
        z = mean_z + epsilon * std
